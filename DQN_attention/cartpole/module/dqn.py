...
def build_attention(e_num=8, patten='multiple', scope="attention"):
...
def mlp(num_layers=1, num_hidden=24, activation=tf.tanh, layer_norm=False):
...
def build_q_func(network, hiddens=[32], dueling=True, layer_norm=False, **network_kwargs):
...
def learn(env, network,
    ...
    data = pd.DataFrame(mean_attention, columns=['a1', 'a2', 'a3', 'a4', 'a5', 'a6', 'a7', 'a8'])
    total_reward = pd.DataFrame(env.episode_reward,columns=['reward'])
    data.to_csv('attention_weigth.csv', index=False)
