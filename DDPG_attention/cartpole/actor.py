...
class ActorNetwork(object):
    ...
    def create_actor_network(self):
        with tf.device(self.device):
            ...
            w3_initial = np.random.uniform(size=(128, self.a_dim), low=-0.0003, high=0.0003).astype(np.float32)
            # Placeholders
    ...
    def save_actor(self):
        ...
        self.saver.save(self.sess, 'log/actor_model.ckpt')
        # print("Model saved in file: actor_model")

    def recover_actor(self):
        self.saver.restore(self.sess, 'log/actor_model.ckpt')
