...
class CriticNetwork(object):
    ...
    def create_critic_network(self, scope):
        with tf.device(self.device):
            ...
            w1_initial = np.random.normal(size=(self.s_dim, 256)).astype(np.float32)
            w2_initial = np.random.normal(size=(256, 128)).astype(np.float32)
            w2_action = np.random.normal(size=(self.a_dim, 128)).astype(np.float32)
            w3_initial = np.random.uniform(size=(128, 1), low=-0.0003, high=0.0003).astype(np.float32)
            # Placeholders
            inputs = tf.placeholder(tf.float32, shape=[None, self.s_dim])
            action = tf.placeholder(tf.float32, shape=[None, self.a_dim])
            # Attention
            # with tf.variable_scope("attention"):
            # e_w = tf.truncated_normal([self.s_dim, 12], stddev=0.1)
            # e_b = tf.constant(0.1, shape=[12])
            # E_W = tf.Variable(e_w)
            # E_b = tf.Variable(e_b)
            ...
            # multiple
            a_w = tf.truncated_normal([self.s_dim, self.s_dim, 2], stddev=0.1)
            ...
            for i in range(self.s_dim):
                attention_FC = tf.matmul(inputs, A_W_unstack[i]) + A_b_unstack[i]
