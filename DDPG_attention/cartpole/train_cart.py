...
def trainer(epochs=1000, MINIBATCH_SIZE=64, GAMMA=0.99, epsilon=1.0, min_epsilon=0.01, BUFFER_SIZE=10000,
    ...
    with tf.Session() as sess:
        ...
        # Creating agent
        ruido = OUNoise(action_dim, mu=0.2)  # this is the Ornstein-Uhlenbeck Noise
        ...
        for i in range(epochs):
            ...
            attention_weight = [[1, 1, 1, 1, 1, 1, 1, 1]]
            weights = []
            done = False
            states = []
            rewards = []
            ...
            while not done:
                ...
                action = np.clip(np.random.normal(action_original, np.max([var, 0.01])), env.min_action, env.max_action)
                ...
                if train_indicator:
                    ...
                    if replay_buffer.size() > MINIBATCH_SIZE:
                        ...
                        # weights = np.array(weights).reshape(-1,6)
                        ...
                        # o_batch = attention_weight * s_batch
                ...
                step += 1
                # pd.DataFrame(weights).to_csv('attention_weights.csv', header=['a1', 'a2', 'a3', 'a4', 'a5', 'a6'], index=False)
                if step > (max_step -1):
                    break
            ...
            if done or step > (max_step - 1):
                if weights:
                    ...
                    # five_weights = np.array(five_weights).reshape((len(weights), 6))
                    ...
                    mean_reward = np.mean(five_reward[-51:])
                    ...
                    if save_reward == None or mean_reward > save_reward:
                        ...
                        # pd.DataFrame(five_weights).to_csv('attention_weights.csv', header=['a1', 'a2', 'a3', 'a4', 'a5', 'a6'], index=False)
                    ...
                    ruido.reset()
                    # print('th', i + 1, 'weights:', five_weights[-1])
        ...
        five_weights = np.array(five_weights).reshape((-1, 8))
        pd.DataFrame(five_weights).to_csv('attention_weights.csv', header=['a1', 'a2', 'a3', 'a4', 'a5', 'a6', 'a7', 'a8'], index=False)
            # if render:
            #     env.render()
