...
from acrobot_att import AcrobotEnv
...
def trainer(epochs=1000, MINIBATCH_SIZE=64, GAMMA=0.99, epsilon=1.0, min_epsilon=0.01, BUFFER_SIZE=10000,
    ...
    with tf.Session() as sess:
        ...
        for i in range(epochs):
            ...
            if done:
                ruido.reset()
                if -state[2] - np.cos(state[1] + state[0]) > 1.:
                    goal += 1
                # if state[0] > 0.45:
                #     goal += 1
            ...
            # if step > 50:
                ...
                # five_weights = np.array(five_weights).reshape((len(weights), 10))
                # four_states.append(states)
                # four_states = np.array(four_states).reshape((len(states), 10))
                states = np.array(states).reshape((len(states), 10))
                actions = np.array(actions).reshape((len(actions), 1))
                # print(np.shape(states))
                all_states = np.append(all_states, [states])
                # all_states.append(four_states)
                ...
                if save_reward == None or mean_reward > save_reward:
                    ...
                    # pd.DataFrame(five_weights).to_csv('attention_weights.csv', header=['a1', 'a2', 'a3', 'a4', 'a5', 'a6', 'a7', 'a8', 'a9', 'a10'], index=False)
        ...
        # all_states = np.array(all_states).reshape((-1, 10))
        all_states = all_states.reshape(-1,10)
        # all_states = np.array(all_states).reshape(1000, 5)
        # all_rewards = np.array(all_rewards).reshape(1000, 1)
        # all_rewards = np.array(all_rewards).reshape((len(rewards), 1))
        pd.DataFrame(all_states).to_csv('all_state.csv', header=['a1', 'a2', 'a3', 'a4', 'a5', 'a6', 'a7', 'a8', 'a9', 'a10'], index=False)
...
if __name__ == '__main__':
    trainer(epochs=1000, epsilon=1., render=False)
