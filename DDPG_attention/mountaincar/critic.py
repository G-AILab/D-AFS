...
class CriticNetwork(object):
    ...
    def create_critic_network(self, scope):
        with tf.device(self.device):
            ...
            w1_initial = np.random.normal(size=(self.s_dim, 48)).astype(np.float32)
            w2_initial = np.random.normal(size=(48, 48)).astype(np.float32)
            w2_action = np.random.normal(size=(self.a_dim, 48)).astype(np.float32)
            w3_initial = np.random.uniform(size=(48, 1), low=-0.0003, high=0.0003).astype(np.float32)
            # Placeholders
            inputs = tf.placeholder(tf.float32, shape=[None, self.s_dim])
            action = tf.placeholder(tf.float32, shape=[None, self.a_dim])
            # Attention
            # with tf.variable_scope("attention"):
            e_w = tf.truncated_normal([self.s_dim, 10], stddev=0.1)
            e_b = tf.constant(0.1, shape=[10])
            E_W = tf.Variable(e_w)
            E_b = tf.Variable(e_b)
            ...
            # multiple
            a_w = tf.truncated_normal([self.s_dim, 10, 2], stddev=0.1)
